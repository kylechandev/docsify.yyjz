...
2. 打开 支付宝app 或 微信 app，进入账单界面，点进任意一个账单明细界面。

3. 此时会自动识别界面信息归纳出账单，并自动弹出记账面板，会自动填充一些基本信息，例如账单日期、类型、金额等，可以再手动调整后确认保存账单。
...
若自动记账识别失败，会在本地存储相关失败的日志信息，且仅存储在本地，可以在「自动记账 - 自动记账日志」看到。如果遇到无法识别的场景，敬请联系我们～ 将无法识别的界面截图或者将失败日志发送给我们。
...
因为 支付宝 app 和 微信 app 在包红包后，没有支付完成界面，所以自动记账无法识别归纳账单信息，需要点开红包后，来识别红包明细界面。
