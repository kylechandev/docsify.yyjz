...
* 建议根据 App 内自动记账设置提示，设置好 App 的保活措施。
...
本节说明如何检查自动记账服务是否成功运行，或如何处理自动记账服务不稳定：

1. 确保 **无障碍** 和 **悬浮窗-显示在其他应用上方** 权限已经授予；

2. 确保 **通知** 权限已经授予，并可以在通知栏查看到“自动记账服务正在运行”的通知；

3. 确保已设置 **允许自启动**，**后台弹出界面**，**电池优化设置为无限制**，**允许受限制的设置**(如果有，例如 HyperOS2.0+)；

4. 尝试重启手机，重启 App，关闭/重新开启 无障碍 权限；

5. 尝试在自动记账设置界面，启用 **自动记账日志**，并跳转到支付宝/微信账单明细界面，再返回 App 后点击 自动记账日志 观察是否有日志产生；

6. 尝试按主屏幕键返回桌面，再切换到需要识别记账的界面；

7. 如果有使用其他无障碍权限的同类 App，请先尝试关闭，防止互相产生干扰；
...
### 部分自动记账App的账单明细有时无法识别？

有可能是因为账单信息过多，「日期」信息是被折叠起来的，此时会导致无法识别到日期。

因为是处在账单明细中，所以必须在识别到账单日期后，才会弹出自动记账面板。
...
1. 如果账单界面有「更多」「展开」按钮，点击它将账单日期显示出来。

2. 如果展开后仍然没有弹出自动记账，可返回手机桌面，再切回到App，此时会再触发自动记账。
